...
- **Politique de confidentialité pas vraiment publiée** : `public/privacy-policy.html` existe (bien rédigée, à jour) mais elle n'est **liée nulle part dans l'app** et n'est hébergée sur aucune URL publique — or Play Console et App Store exigent tous les deux une URL live à renseigner dans la fiche store. Le plus simple serait de l'héberger sur Firebase Hosting (déjà utilisé pour Firestore, même projet) et d'ajouter un lien vers elle dans les Options.
