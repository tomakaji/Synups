...
- ~~**Politique de confidentialité pas vraiment publiée**~~ **Fait côté code** : `firebase.json`/`.firebaserc` configurent Firebase Hosting sur ce même projet, et un lien "Politique de confidentialité" a été ajouté dans Options (pointe vers `https://synups-3b038.web.app/`). **Reste à faire (toi seul peux le faire, ça demande ta connexion Google)** : depuis le dossier du projet, lancer `firebase login` puis `firebase deploy --only hosting` pour que cette URL devienne réellement en ligne — sans ce déploiement, le lien pointe vers une page qui n'existe pas encore.
